...
void CTalkDialog::OnBnClickedButtonStartTalk()
{
	...
	VIDEO_TALK_REQ req = { 0 };
	req.pUserData = this;
	req.appkey = nullptr;
	req.secretkey = nullptr;
	req.fnMsg = &CTalkDialog::cb_TalkMsg;
	...
	int iRet = isAsyn ? Video_StartTalk(url.c_str(), nullptr, &req) : Video_StartAsynTalk(url.c_str(), nullptr, &req);
